#!/bin/sh
# Play a file, playlist, or URL with bundled VLC (if present) or CrossMix mpv.
...
if [ -x "$GPTK" ]; then
  "$GPTK" -1 "mpv" -c "$APPDIR/keys.gptk" &
  GPTK_PID=$!
fi
if [ -x /mnt/SDCARD/System/bin/thd ] && [ -f "$APPDIR/thd.conf" ]; then
  /mnt/SDCARD/System/bin/thd --triggers "$APPDIR/thd.conf" /dev/input/event3 &
  THD_PID=$!
fi

# Optional real VLC drop-in: Apps/VLC/bin/vlc (+ libs in Apps/VLC/bin/lib)
if [ -x "$APPDIR/bin/vlc" ]; then
  export LD_LIBRARY_PATH="$APPDIR/bin/lib:${LD_LIBRARY_PATH:-}"
  "$APPDIR/bin/vlc" --fullscreen --play-and-exit --no-qt-privacy-ask "$TARGET"
  exit $?
fi
